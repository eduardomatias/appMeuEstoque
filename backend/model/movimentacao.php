...
class Movimentacao extends Model {
    ...
    public function getMovimentacao($empresa) {
        ...
        $sql = "
                ...
                ORDER BY TBL04_DATA DESC,TBL04_ID DESC";
     ...
     * @return array
     */
    public function getEstoqueAtual($empresa) {
        $tableSchema = $this::tableSchema();
        $tableName = $this::tableName();
        $sql = "
                SELECT TBL02_NOME AS PRODUTO, TBL02_ATIVO, SUM(TBL04_QTD) AS QTD, SUM(TBL04_CUSTO_TOTAL) AS VALOR
                FROM " . $tableSchema . "." . $tableName . "
                INNER JOIN $tableSchema.TBL02_PRODUTO ON (TBL02_ID = TBL04_ID_PRODUTO)
                WHERE TBL04_ID_EMPRESA = ?
                GROUP BY TBL02_NOME, TBL02_ATIVO
                ORDER BY TBL02_NOME";
        return $this->db->queryAssoc($sql, array($empresa));
    }
